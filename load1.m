...
ny=(P*delta+Y)/m/g;
% 压心(对最前边取矩)
cz_0=[c(1) frame(1)+tube(1)/2 frame(2)+c(2) frame(3)+tube(2)/2 ...
      frame(11)+c(3) frame(12)+tube(3)/2 frame(end-1)+c(4)];
...
%% 绘制剪力、弯矩
A= [3.27, 3.85, 1.61, 0.25, 3.58, 0.3501, 1.09, 2.49, 2.0, 2.8, 0.3502, 2.33, 4.97, 1.08, 2.28, 3.01, 1.69, 2.246, 3.004, 4.03];
B0 = zeros(2*size(A,1)-1, 1);
for i = 1:size(A,1)-1
